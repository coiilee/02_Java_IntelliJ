...
public class For문 {
    ...
    public void method2() {
        ...
        2차원 데이터 처리나 중첩된 작업을 진행할 때 사용함
        ex)구구단

        for(초기식1; 조건식1; 증감식1) {     //외부 반복분

        //조건식1이 true일 때 반복 실행될 코드
        for(초기식2; 조건식2; 증감식2) {     //내부 반복문
            //조건식 2가 true 일때 반복 실행될 코드
        }
        ...
        * */
        for (int i=0; i<10; i++) {
            for (int j=0; j<10; j++) {
            System.out.println("i의 값: "+i+" ,j의 값 : "+i);
            }
        }
    }
    ...
    public void method3() {
        //구구단은 2단부터 시작하니까 초기식 시작 2 (i=2;)
        for (int i=2; i<10; i++) {
            System.out.println("=====" +i+ "단=====");

            //구구단은 단 옆 반복할 수를 1부터 시작하기 때문에 초기식 1부터 시작 (j=1;)
            for (int j=1; j<10; j++) {
                System.out.println(i+ "x" + j + "=" + i *j);
            }
            System.out.println();//한칸 줄바꿈 해서 단이 변경될 때마다 한줄 공백처리
        }
    }
    ...
    public void method4() {
        ...
        자바 5버전에서부터 도입된 반복문 **우리는 현재 java11버전 사용중
        배열이나 컬렉션과 같은 데이터 집합을 다룰때 사용
        기존의 for 문보다 간결하고, 반복문에서 인덱스처리가 필요하지 않을 때 사용함
        ...
        **요소 : html 에서는 <태그>내용물</태그>
                ...
                numbers의 배열 요소 : 10,20,30,40 (와 같이 안에 들어있는 값들)
